...
if (!empty($_POST)) {
  ...
  if (empty($_POST['producto']) || empty($_POST['precio'])) {
    ...
    $query_update = mysqli_query($conexion, "UPDATE producto SET descripcion = '$producto', precio= $precio,existencia= $existencia WHERE codproducto = $codproducto");
    if ($query_update) {
      $alert = '<div class="alert alert-primary" role="alert">
              Modificado
            </div>';
...
if (empty($_REQUEST['id'])) {
  ...
  $query_producto = mysqli_query($conexion, "SELECT codproducto, descripcion, precio, existencia FROM producto   WHERE codproducto = $id_producto");
