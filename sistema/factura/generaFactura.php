<?php
    ...
    if(empty($_REQUEST['cl']) || empty($_REQUEST['f']))
    {
        ...
        $pdf->Ln();
        $pdf->image("img/lo.jpeg", 55, 18, 15, 10 );
        ...
        $pdf->SetFont('Arial', 'B', 7);
        $pdf->Cell(16, 10, "Fecha: ", 0, 0, 'R');
        $pdf->SetFont('Arial', '', 7);
        $pdf->Cell(25, 10, $result_venta['fecha'], 0, 1, 'R');
        $pdf->SetFont('Arial', 'B', 7);
        $pdf->Cell(60, 5, "Datos del cliente", 0, 1, 'L');
        $pdf->Cell(30, 5, "Nombre", 0, 1, 'L');
       
        $pdf->SetFont('Arial', '', 7);
        $pdf->Cell(40, 5, utf8_decode($result_cliente['nombre']), 0, 1, 'L');
        ...
        $pdf->Cell(65, 5, 'Total: ' . number_format($result_venta['totalfactura'], 2, '.', ','), 0, 1, 'R');
        $pdf->Ln();
        $pdf->SetFont('Arial', '', 7);
