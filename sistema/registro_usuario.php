...
if (!empty($_POST)) {
    $alert = "";
    if (empty($_POST['correo']) || empty($_POST['usuario']) || empty($_POST['clave']) || empty($_POST['rol'])) {
        ...
        if ($result > 0) {
            ...
            $query_insert = mysqli_query($conexion, "INSERT INTO usuario(usuario, correo, clave,rol) values ('$user','$email', '$clave', '$rol')");
            if ($query_insert) {
                $alert = '<div class="alert alert-primary" role="alert">
                            Usuario registrado
                        </div>';
